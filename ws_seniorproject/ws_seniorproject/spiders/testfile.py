...
class TestfileSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        urls = response.css('h2.a-size-mini.a-spacing-none.a-color-base.s-line-clamp-2 a').xpath('@href')[0:20].extract()
    ...
    def parse_link(self, response):
        ...
        if found_dimension == True:
            try:
                ...
                dim_len = 'Dimensions unavailable'
        if found_dimension == True:
            try:
                ...
                dim_width = 'Dimensions unavailable'
        if found_dimension == True:
            try:
                ...
                dim_height = 'Dimensions unavailable'
        ...
        try:
            ...
            items['Rank_Category'] = 'Category Unavailable'

        try:
            items['Answered_Questions'] = answered
        except UnboundLocalError:
            items['Answered_Questions'] = 'Unavailable'
        ...
        try:
            ...
            items['Dimensions'] = 'Dimensions unavailable'

        try:
            items['Dimensions_Length'] = dim_len
        except UnboundLocalError:
            items['Dimensions_Length'] = 'Dimensions unavailable'

        try:
            items['Dimensions_Width'] = dim_width
        except UnboundLocalError:
            items['Dimensions_Width'] = 'Dimensions unavailable'

        try:
            items['Dimensions_Height'] = dim_height
        except UnboundLocalError:
            items['Dimensions_Height'] = 'Dimensions unavailable'

        try: 
            items['ASIN_Number'] = asin_number
        except UnboundLocalError:
            items['ASIN_Number'] = 'ASIN unavailable'

        try:
            items['First_Listed'] = first_listed
        except UnboundLocalError:
            items['First_Listed'] = 'First listed attribute unavailable'
        ...
        dim_height=dim_height, first_listed=first_listed, Fit_As_Expected=fit, Days_Since_First_Listed=days, asin_num=asin_number)
